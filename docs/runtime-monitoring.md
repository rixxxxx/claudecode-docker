...
output, and desktop notification all confirmed working; see
[AGENTS.md](../AGENTS.md#runtime-monitoring) for the verification history
across Falco upgrades.
...
See [falco/claude-code-rules.yaml](../falco/claude-code-rules.yaml), plus
Falco's own bundled default ruleset:
...
- A non-Claude process reading claude-code's OAuth credentials file —
  ...
  (see the [README's Persistence section](../README.md#persistence)).
...
- A network tool (`nc`, `socat`, `tcpdump`, ...) launched with an
  ...
  `NPM_CONFIG_IGNORE_SCRIPTS=true` (Dockerfile default, see
  [.env.example](../.env.example) to opt out per workspace) — npm
  lifecycle scripts don't run at all by default, so there's usually
  nothing here for this rule to catch.
...
Natively, via your desktop's own notification
...
change — it can't silence its own alarm. See
...
again. See [.env.example](../.env.example).

### Automatic stop on repeated CRITICAL alerts

`cc-container --monitor`
...
actually stop it, never this one. See
[AGENTS.md](../AGENTS.md#runtime-monitoring) for the mechanics.
...
CRITICAL alert Falco produces host-wide (see
[AGENTS.md](../AGENTS.md#runtime-monitoring) for why that distinction
matters). Once stopped, `claude-code` stays stopped (`restart:
unless-stopped` doesn't restart a manually-stopped container) — start it
again the normal way (`cc-container`) when you're ready to investigate.

### Testing without waiting for a real violation

[falco/claude-code-rules.yaml](../falco/claude-code-rules.yaml)
...
assistant-issued Bash tool call); see
[AGENTS.md](../AGENTS.md#runtime-monitoring) for that manual procedure and
...
- `security-monitor` is the one service in this repo that runs with
  ...
  itself gets nothing extra from this (see
...
Limits of syscall-based detection, not bugs to fix:
...
2. **Live-verify it before trusting it**, using this repo's own
   established workflow (see [AGENTS.md](../AGENTS.md) for the general
   procedure): add
