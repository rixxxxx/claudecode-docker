...
test_ntlm_uses_proxy_auth_sidecar() {
    ...
    assert_contains "$(cat "$px_ini")" "password = s3cr3t"
    assert_equal true "$NEEDS_PROXY_AUTH_SIDECAR"
}

test_kerberos_without_password_omits_password_line() {
...
run_test test_kerberos_without_password_omits_password_line
